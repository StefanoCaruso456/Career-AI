...
- Rewrote the top-level README to describe the repo as a real Next.js application plus workspace packages and optional services, not a frontend foundation.
...
- Corrected A2A docs to match the real protocol persistence tables and the real auth model.
...
- Several UI routes remain mostly placeholder shells: `/account/settings`, `/employer/roles`, and `/employer/agent-sorcerer`.
...
- Production deployment behavior beyond the checked-in Railway and workflow files was not directly observed.
