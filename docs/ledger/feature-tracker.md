...
- [ ] **Career ID badges row (plain-JSON MVP)** — render `careerIdProfile.badges` as a horizontal chip strip above the phase list. Collapses today's hardcoded per-card "Government ID verified" / "Offer letter verified" pills into one generalized badge surface. Plain JSON only — no VC / crypto. Upgrades cleanly when spec 005's wallet lands (add optional `credentialId` + click-to-verify). See spec [`006-career-id-badges-row.md`](./planning/specs/006-career-id-badges-row.md).
